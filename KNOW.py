...
# 测试文件路径
excelpath=os.getcwd()+'/test-data/知识图谱测试.xlsx'
# 测试结果文件路径
testresult=os.getcwd()+'/test-results/kg-results.xlsx'

# 测试接口url
url='http://172.16.100.74:8080/v1/openapi'
    ...
    def run(self):
        ...
        while len(questionsCollection) > 0:
            ...
            header = {'Content-Type': 'application/json', "cache-control": "no-cache",
                      "Postman-Token": "24786e26-bdb4-47a6-8150-01bdae22728d", 'userId': str(gl.getTimestamp()),
                      'appId': 'csbot',
                      }
            # body（JSON） 信息
            ...
            flag=False
            try:
                # actualAnswer = resposeJsonStdQ['answer_module']['intent']
                actualAnswer = resposeJsonStdQ['data'][0]['value']
            except BaseException:
                flag=True

            if flag==False:
                if gl.repaceSpecialCharactersinString(actualAnswer) == gl.repaceSpecialCharactersinString(stdA):
                    # print('pass')
...
multiThreadCount=3 # set threads to ran the test
