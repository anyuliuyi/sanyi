...
excelpath=os.getcwd()+'/test-data/faq客户3.12.xlsx'
...
class cxThreadWorker(threading.Thread):
    ...
    def run(self):
        ...
        while len(questionsCollection) > 0:
            ...
            # 头信息  # 纯faq的appid:8910a62a68cb47f99a3eafc7f5000b8a
            # 测试环境的appid:b6ad1f823d8d4ffea36568330239f237
            header = {'Content-Type': 'application/json', 'sessionId': str(gl.getTimestamp()),
                      'userId': str(gl.getTimestamp()), 'appId': 'csbot',
                      }
            # body（JSON） 信息
...
multiThreadCount=2 # set threads to ran the test
